...
void readMPURaw() {
  static uint64_t last_good_packet_ts;
  // Reset the MPU interrupt flag
  mpuIntStatus = false;
  // Update the FIFO size counter
  fifoCount = mpu.getFIFOCount();
  // Check if Data Ready interrupt is (1 for interrupt generated, 0 otherwise)
  bool dataReady = mpu.getIntDataReadyStatus();
}
...
void MPU6050Connect() {
  ...
  if (devStatus != 0) {
    ...
    if (MPUInitCtr >= 10) return;  //only try 10 times
    delay(1000);
  ...
  dmpReady = true;
  #else
  Serial.println(F("Initializing raw MPU..."));
  mpu.initialize();
  packetSize = mpu.getFIFOCount();
  #endif
  ...
  // Activate arduino interrupt edge detection
...
void setup() {
  ...
  i2cSetup();
  Serial.println(F("MPU6050 setup..."));
  MPU6050Connect();
